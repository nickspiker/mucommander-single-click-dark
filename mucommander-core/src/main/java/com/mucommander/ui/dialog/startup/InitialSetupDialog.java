...
public class InitialSetupDialog extends FocusDialog implements ActionListener {
    ...
    private JPanel createThemePanel() {
        ...
        // Use whatever the current/default theme is (Native + FlatLaf Dark works well)
    ...
    private JPanel createLookAndFeelPanel() {
        ...
        // ALWAYS prefer FlatLaf Dark for default (no more blinding!)
        int flatDarkIndex = -1;
        for (int i = 0; i < lfInfo.length; i++) {
            if (lfInfo[i].getClassName().contains("FlatDark")) {
                flatDarkIndex = i;
                break;
            }
        }
        // Use FlatDark if found, otherwise use current, otherwise first
        if (flatDarkIndex != -1)
            selectedIndex = flatDarkIndex;
